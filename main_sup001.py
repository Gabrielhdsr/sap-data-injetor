...
def conectar_db():
    try:
        ...
        uri = (
            f"mssql+pyodbc://{user}:{pw}@{cfg['server']}/{cfg['database']}"
...
def obter_header_tecnico(ws):
    """
    Lê a linha 5 (índice 4) da tabela da aba: nomes técnicos dos campos.
    """
...
def obter_tipos_coluna(ws):
    """
    Lê a linha 6 (índice 5) da tabela da aba: tipo de dado (Texto, Número, Data, etc.).
    """
    table = ws.find(".//ss:Table", namespaces=NS)
    if table is None:
        return []
...
def is_numeric_column(tipo_str):
    """
    Marca como numérica apenas:
    - colunas com texto 'Número/Numero', ou
    - linhas técnicas que começam com 'ENU;' (ex.: ENU;13;3;P;13;3).
    Todo o resto (ex.: ETE;80;0;C;80;0) fica como texto.
    """
    if not tipo_str:
        return False

    t = tipo_str.upper().strip()

    if "NÚMERO" in t or "NUMERO" in t:
        return True
...
def to_number_for_xml(value):
    """
    Converte strings tipo '6,200', '10,000', '0,000' para formato numérico com ponto.
    Se não conseguir, devolve None para deixar célula vazia.
    """
    if value is None:
        return None
    s = str(value).strip()
    ...
    # troca vírgula por ponto e remove separadores de milhar
    s = s.replace(".", "").replace(",", ".")
    try:
        n = float(s)
        if n.is_integer():
...
def preencher_aba_xml(ws, df, header_xml):
    """
    Remove linhas de dados >= 9 e recria com base em header_xml,
    tratando colunas numéricas como Number e ajustando ExpandedRowCount.
    """
    table = ws.find(".//ss:Table", namespaces=NS)
    rows = table.findall("ss:Row", namespaces=NS)

    # apaga somente linhas a partir da 9 (índice 8)
    for r in range(len(rows) - 1, 7, -1):
        table.remove(rows[r])

    # tipos por coluna para decidir String vs Number
    tipos = obter_tipos_coluna(ws)
    if len(tipos) < len(header_xml):
        tipos = tipos + [""] * (len(header_xml) - len(tipos))
    ...
    for _, row in df.iterrows():
        ...
        for idx, col_name in enumerate(header_xml):
            ...
            tipo_col = tipos[idx] if idx < len(tipos) else ""
            is_num = is_numeric_column(tipo_col)

            cell = etree.SubElement(row_node, _q("Cell"), Index=str(idx + 1))
            data_attrs = {}

            if is_num:
                num_txt = to_number_for_xml(val)
                if num_txt is None:
                    data_attrs[f"{{{NS['ss']}}}Type"] = "String"
                    data_text = ""
                else:
                    data_attrs[f"{{{NS['ss']}}}Type"] = "Number"
                    data_text = num_txt
            else:
                data_attrs[f"{{{NS['ss']}}}Type"] = "String"
                data_text = xml_safe(val)

            etree.SubElement(cell, _q("Data"), data_attrs).text = data_text

    # ajusta ExpandedRowCount para refletir nº real de linhas
    todas_rows = table.findall("ss:Row", namespaces=NS)
    table.set(_q("ExpandedRowCount"), str(len(todas_rows)))
...
def processar_layout(nome_input):
    ...
    layout_path = os.path.join(
        "layouts",
        nome_input if nome_input.endswith(".xml") else f"{nome_input}.xml"
    )
    ...
    for ws in tree_template.xpath("//ss:Worksheet", namespaces=NS):
        ...
        if any(ign in aba_original for ign in ABAS_IGNORADAS):
            continue
        ...
        try:
            ...
            log_msg(
                log,
                f"[FALHA] Tabela não encontrada: {nome_tabela}",
                console=False,
            )

    if not mapa_execucao:
        log_msg(log, "\n[ERRO] Nenhuma tabela vinculada.")
    ...
    aba_mestre = next(
        (aba for aba in mapa_execucao if "DADOS_GERAIS" in normalizar_nome_tabela(aba)),
        list(mapa_execucao.keys())[0],
    )
    ...
    try:
        ...
        df_ids = pd.read_sql(
            f"SELECT DISTINCT {CHAVE_DETECTADA} "
            f"FROM dbo.[{tabela_mestre}] ",
            engine,
        )
        ids_unicos = df_ids[CHAVE_DETECTADA].dropna().unique().tolist()
        try:
            ids_unicos.sort(key=int)
        except Exception:
            ids_unicos.sort()
    ...
    for i in range(lotes):
        ...
        for aba, meta in mapa_execucao.items():
            ...
            try:
                query = (
                    f"SELECT * FROM dbo.[{tabela}] "
                    f"WHERE {CHAVE_DETECTADA} IN ('{ids_sql}')"
                )
                df = (
            ...
            if CHAVE_DETECTADA in df.columns:
                df = df.sort_values(by=CHAVE_DETECTADA)
        ...
        xml_str = etree.tostring(
            tree_lote, encoding="unicode", xml_declaration=False
        ).replace('<?mso-application progid="Excel.Sheet"?>', "")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('Uso: python main.py "ARQUIVO.xml"')
    else:
        processar_layout(sys.argv[1])
